...
def install_local_qwen_pipeline():
    ...
    # The base CopilotAI already starts a background Ollama warmup. Keep the real answer
    # context aligned with that warmup so Ollama does not immediately rebuild the runner
    # at a larger context size after reporting that the model was warmed.
    os.environ["OLLAMA_WARMUP"] = "1"

    original_init = CopilotAI.__init__
    original_settings_init = SettingsDialog.__init__

    def low_latency_ctx(requested):
        cap = _int_env("LOCAL_QWEN_CONTEXT_TOKENS", 4096, 2048, 8192)
        return max(2048, min(int(requested or cap), cap))

    def ai_init(ai, provider=None):
        original_init(ai, provider="ollama")
        ai.provider_preference = "ollama"
        ai.ollama.num_ctx = low_latency_ctx(ai.ollama.num_ctx)
        ai.omni_vision = NvidiaOmniVisionClient(
            api_key=os.environ.get("NVIDIA_API_KEY", "")
        )
        print(
            f"[pipeline] Final answer engine: local {ai.ollama.model}; "
            f"context={ai.ollama.num_ctx}; vision extractor: {ai.omni_vision.model}"
        )
    ...
    def set_config(ai, provider=None):
        ...
        ai.ollama.reconfigure(
            base_url=base_url,
            model=local_model,
            num_ctx=low_latency_ctx(requested_ctx),
        )
        ai.provider_preference = "ollama"
    ...
    def verify_qwen_available(ai):
        """Fail fast only when Ollama/model availability is actually broken.

        Do not run another synchronous warmup here. The background startup warmup already
        prepares the model, and repeating a 4096-token warmup directly before every first
        real answer adds avoidable latency.
        """
    ...
    def bounded_qwen_stream(ai, prompt, max_tokens, image_bytes_list=None):
        ...
        if image_bytes_list:
            ...
            first_timeout = _float_env(
                "LOCAL_QWEN_FIRST_TOKEN_TIMEOUT_SECONDS", 45.0, 30.0, 120.0
            )
        ...
        while True:
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "Run `ollama ps` while this request is active and inspect PROCESSOR/CONTEXT."
                )
            if event_type == "chunk":
                if first:
                    ...
                    print(
                        ...
                        f"model={ai.ollama.model} · context={ai.ollama.num_ctx}"
                    )
                    first = False
                yield payload
    ...
    def generate_text_stream(ai, custom_query=None):
        ...
        print(
            f"[pipeline] Transcript/chat -> local Qwen · context={ai.ollama.num_ctx} · "
            f"prompt_chars={len(prompt)}"
        )
        yield from bounded_qwen_stream(ai, prompt, max_tokens=max_tokens)

    def fast_visual_text_prompt(task, visual_text):
        # Manual screen questions are usually standalone coding/MCQ/debugging tasks. Do
        # not prepend the full interview knowledge base/transcript to every visual solve;
        # that increases prefill latency without helping HackerRank-style questions.
        return (
            "Solve the practice question from the extracted screen text below. "
            "Answer immediately and do not repeat the problem statement. "
            "For an MCQ, put the correct option and answer on the first line. "
            "For coding, give a very short approach, then complete correct code in the requested language, "
            "then time and space complexity. For debugging, state the defect and corrected code. "
            "Use only readable extracted screen evidence; do not invent missing text.\n\n"
            f"USER TASK:\n{task}\n\n"
            f"SCREEN TEXT:\n{visual_text}"
        )
    ...
    def generate_vision_stream(
        ...
        max_tokens = int(os.environ.get("VISION_MAX_TOKENS", "900"))
        ...
        try:
            ...
            fallback_prompt = (
                "Read the supplied practice screenshot and answer the visible question immediately. "
                "For MCQ put the correct option first. For coding give a short approach, complete correct "
                "code, and time/space complexity. For debugging identify the defect and corrected code. "
        ...
        prompt = fast_visual_text_prompt(task, visual_text)
        print(
            f"[pipeline] NVIDIA visual text -> local Qwen FAST · context={ai.ollama.num_ctx} · "
    ...
    def route_stream(ai, prompt, max_tokens, image_bytes_list=None):
        ...
        if image_bytes_list:
            try:
                ...
                visual_text = validate_visual_text(visual_text)
                prompt = fast_visual_text_prompt(prompt, visual_text)
